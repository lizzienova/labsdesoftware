...
void big_val (BigInt res, long val)
{
    ...
    // copia os bytes do long (little-endian: byte menos significativo em res[0])
    for (int i = 0; i < n; i++) {
        // isola o byte e o armazena em res[i]
        res[i] = (uvalor >> (8 * i)) & 0xFF;
    }
    // extensão de sinal: preenche os bytes restantes
...
{
    /* Complemento a 1 (inversão de bits) */
    for (int i = 0; i < NUM_BYTES; i++) {
        res[i] = ~a[i];
    }
    
    /* Soma 1 com propagação do "vai-um" (little-endian) */
...
void big_sub(BigInt res, BigInt a, BigInt b) {
    BigInt negado_b;
    // calcula o complemento a 2 do subtraendo: negado_b = -b
    big_comp2(negado_b, b);
    // realiza a soma: res = a + negado_b
...
void big_mul(BigInt res, BigInt a, BigInt b) {
    ...
    /* Multiplicação sem sinal: array temporário de 256 bits (32 bytes) */
    unsigned char temp[NUM_BYTES * 2];
    // inicializa 'temp' com zero
...
void big_shl(BigInt res, BigInt a, int n)
{
    ...
    /* Deslocamento por bytes: move os bytes existentes e zera os de entrada. */
    ...
    /* Deslocamento por bits (propagação de "vai-um", da direita para a esquerda) */
    if (n_qtbits > 0) {
...
void big_shr(BigInt res, BigInt a, int n)
{
    ...
    /* Deslocamento por bits (propagação de "vai-um", da esquerda para a direita) */
    if (n_qtbits > 0) {
        unsigned char carry = 0;
